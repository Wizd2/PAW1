<?php
// ================================
// ClickClick — contact.php 
// 3 proste funkcje do strony "Kontakt":
// 1) PokazKontakt() — pokazuje formularz
// 2) WyslijMailKontakt() — wysyla wiadomosc
// 3) PrzypomnijHaslo() — wysyla login/haslo admina (wersja uproszczona)
// ================================

// ClickClick — v1.7 
// contact.php: PokazKontakt(), WyslijMailKontakt(), PrzypomnijHaslo()

// Prosta funkcja do ucieczki znakow (zeby HTML sie nie psul)
function _cc_escape($s): string {
 return htmlspecialchars((string)$s, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
}

// Pobieramy pole z POST i obcinamy spacje
function _cc_post(string $key): string {
 return isset($_POST[$key]) ? trim((string)$_POST[$key]) : '';
}

// Sprawdzenie czy e-mail ma sens (prosta walidacja)
...
function PokazKontakt(): string {
 ...
 <section class="card">
 <h2>Kontakt</h2>
 <p>Wypełnij formularz kontaktowy, a my odpowiemy na podany adres e-mail.</p>
...
function WyslijMailKontakt(): string {
 ...
 // Proba wyslania maila (w XAMPP na Windows czasem to nie dziala bez konfiguracji)
$sent = @mail($to, $safeSubject, $body, implode("\r\n", $headers));
 ...
 To czeste w lokalnym XAMPP. Kod jest poprawny na potrzeby.
 </div><div style="height:12px"></div>';
 $out.= PokazKontakt();
 return $out;
}
...
function PrzypomnijHaslo(): string {
 ...
 $subject = 'ClickClick - przypomnienie hasla ';
 ...
 W XAMPP lokalnie to bywa normalne. Kod jest na potrzeby.
 </div><div style="height:12px"></div>';
