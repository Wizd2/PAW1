...
// --- YouTube helper (embed fix) ---
function cc_youtube_id(string $urlOrId): ?string {
  $s = trim($urlOrId);
  if ($s === '') return null;
  ...
  // /shorts/ID
  if (preg_match('~/shorts/([A-Za-z0-9_-]{11})~', $s, $m)) return $m[1];

  return null;
}

function cc_youtube_embed(string $urlOrId): ?string {
  $id = cc_youtube_id($urlOrId);
  if (!$id) return null;
